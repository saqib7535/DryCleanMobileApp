...
const Modal = (function () {
  ...
  function prompt(opts) {
    opts = opts || {};
    const title = opts.title || I18n.t('common.edit');
    const message = opts.message || '';
    return new Promise((resolve) => {
      const sheet = open(`
        <div class="modal-header"><h3>${title}</h3></div>
        ${message ? `<p style="margin-bottom:10px;color:var(--color-text-muted)">${message}</p>` : ''}
        <div class="field"><input type="text" id="prompt-input" value="${(opts.value || '').replace(/"/g, '&quot;')}" /></div>
        <div class="flex gap-8 mt-8">
          <button class="btn btn-outline btn-block" data-act="cancel">${I18n.t('common.cancel')}</button>
          <button class="btn btn-primary btn-block" data-act="ok">${I18n.t('common.save')}</button>
        </div>
      `, { center: true });
      const input = sheet.querySelector('#prompt-input');
      input.focus();
      sheet.querySelector('[data-act="cancel"]').onclick = () => { close(); resolve(null); };
      sheet.querySelector('[data-act="ok"]').onclick = () => { const v = input.value; close(); resolve(v); };
    });
  }
