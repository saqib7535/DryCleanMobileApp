...
const NewOrderScreen = (function () {
  ...
  async function render(app, params) {
    ...
    if (params && params.id) {
      existingOrder = await OrderService.getById(parseInt(params.id, 10));
      if (!existingOrder) {
        app.innerHTML = `<div class="empty-state"><div class="ei">🔍</div><p data-i18n="order.noOrders"></p></div>`;
        I18n.apply(app);
        return;
      }
      editingOrderId = existingOrder.id;
      selectedCustomer = { id: existingOrder.customer_id, name: existingOrder.customer_name, phone: existingOrder.customer_phone };
      items = existingOrder.items.map((it) => ({
        ...
        quantity: it.quantity, rate: it.rate, rateTouched: true,
        photo_data: it.photo_path || null
      }));
    }

    const trackingPreview = existingOrder ? existingOrder.tracking_no : await OrderService.peekNextTrackingNo();
    ...
    app.innerHTML = `
      <header class="app-header">
        ...
        <div>
          <h1 data-i18n="${existingOrder ? 'order.editTitle' : 'order.newTitle'}"></h1>
      ...
      <div class="page page-pad" style="padding-top:14px;padding-bottom:24px">
        ...
        <div class="card" style="margin-bottom:14px">
          <div class="field-row">
            <div class="field">
              ...
              <input type="date" id="in-order-date" value="${existingOrder ? existingOrder.order_date : Format.todayIso()}" />
            </div>
            <div class="field">
              <label data-i18n="order.returnDate"></label>
              <input type="date" id="in-return-date" value="${existingOrder && existingOrder.return_date ? existingOrder.return_date : ''}" />
            </div>
          </div>
          <div class="checkbox-row">
            <input type="checkbox" id="in-urgent" ${existingOrder && existingOrder.urgent ? 'checked' : ''} />
        ...
        <div class="card" style="margin:14px 0">
          <div class="field-row">
            <div class="field"><label data-i18n="order.discount"></label><input type="number" min="0" id="in-discount" value="${existingOrder ? existingOrder.discount : 0}" /></div>
            <div class="field"><label data-i18n="order.extraCharges"></label><input type="number" min="0" id="in-extra" value="${existingOrder ? existingOrder.extra_charges : 0}" /></div>
          </div>
          <div class="field"><label data-i18n="order.deliveryCharges"></label><input type="number" min="0" id="in-delivery" value="${existingOrder ? existingOrder.delivery_charges : 0}" /></div>
        ...
        <div class="card" style="margin-bottom:14px">
          <div class="field-row">
            <div class="field"><label data-i18n="order.advancePaid"></label><input type="number" min="0" id="in-advance" value="${existingOrder ? existingOrder.advance_paid : 0}" /></div>
            <div class="field">
        ...
        <div class="field">
          ...
          <textarea id="in-notes" rows="2">${existingOrder ? escapeHtml(existingOrder.notes || '') : ''}</textarea>
        </div>

        <button class="btn btn-primary btn-block mt-8" id="btn-save-order" data-i18n="${existingOrder ? 'order.updateOrder' : 'order.saveOrder'}"></button>
      </div>
    `;
    I18n.apply(app);

    app.querySelector('#btn-back').onclick = () => Router.navigate(existingOrder ? '/orders/' + existingOrder.id : '/dashboard');
    app.querySelector('#btn-add-item').onclick = () => { addItemRow(); renderItems(); };
    app.querySelector('#btn-save-order').onclick = save;

    if (existingOrder) app.querySelector('#in-payment-method').value = existingOrder.payment_method || 'Cash';
