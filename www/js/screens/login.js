...
const LoginScreen = (function () {
  ...
  async function render(app) {
    ...
    const defaultCredsHidden = await SettingsService.get('default_creds_hidden', '0');

    app.innerHTML = `
      <div class="login-screen">
        <div class="login-card">
          <div class="login-logo">${Icons.svg('user', 34)}</div>
          ...
          <form id="login-form" novalidate>
            ...
            ${defaultCredsHidden !== '1' ? `<p class="center text-muted mt-16" style="font-size:12px" data-i18n="login.defaultCreds"></p>` : ''}
          </form>
        </div>
      </div>
    `;
    ...
    app.querySelector('#login-form').addEventListener('submit', async (e) => {
      ...
      const minutes = await SettingsService.get('auto_logout_minutes', '0');
      AuthService.setAutoLogoutMinutes(parseInt(minutes, 10));
