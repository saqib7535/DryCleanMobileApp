...
const ReportService = (function () {
  ...
  async function profitReport(fromDate, toDate) {
    const revRows = await DB.query(`
      SELECT COALESCE(SUM(grand_total),0) AS revenue, COUNT(*) AS orders
      FROM orders WHERE order_date BETWEEN ? AND ? AND status != 'Cancelled'
    `, [fromDate, toDate]);
    const revenue = revRows[0].revenue;
    const orderCount = revRows[0].orders;
    ...
    `, [fromDate, toDate]);
    const totalExpenses = sum(expByCat, 'amount');
    const netProfit = revenue - totalExpenses;

    const rows = [
      { label: 'Total Revenue', amount: revenue },
      ...expByCat.map((e) => ({ label: 'Expense — ' + e.category, amount: -e.amount })),
      { label: 'Net Profit', amount: netProfit }
    ];

    return {
      title: `Profit Report — ${fromDate} to ${toDate}`,
      columns: [{ key: 'label', label: 'Item' }, { key: 'amount', label: 'Amount', money: true }],
      rows,
      totals: [
        { label: 'Revenue', value: revenue, money: true },
        { label: 'Expenses', value: totalExpenses, money: true },
        { label: 'Net Profit', value: netProfit, money: true },
  ...
  async function generate(type, params) {
    ...
    switch (type) {
      ...
      case 'profit': return profitReport(params.from || Format.todayIso().slice(0, 8) + '01', params.to || Format.todayIso());
