...
const ReportsScreen = (function () {
  const TYPES = ['daily', 'weekly', 'monthly', 'yearly', 'orderStatus', 'customer', 'invoiceCustomer', 'invoiceDate', 'revenue', 'pending', 'delivered', 'category', 'service', 'expense'];
  ...
  function needsParams(type) {
    return ['daily', 'weekly', 'monthly', 'yearly', 'revenue', 'delivered', 'orderStatus', 'invoiceCustomer', 'invoiceDate', 'expense'].includes(type);
  }
  ...
  function paramsHtml(type) {
    ...
    if (type === 'revenue' || type === 'delivered' || type === 'expense') {
      return `
        <div class="field-row">
  ...
  function readParams(type) {
    ...
    if (type === 'revenue' || type === 'delivered' || type === 'expense') return { from: document.getElementById('p-from').value, to: document.getElementById('p-to').value };
  ...
  function buildReportPdfThermal(report, paperSize) {
    ...
    const perRow = 4 + report.columns.length * 4.4;
    const estHeight = 26 + report.totals.length * 4.4 + report.rows.length * perRow;
    const doc = new jsPDF({ unit: 'mm', format: [width, Math.max(60, estHeight)] });
    const marginX = 4;
    let y = 7;
