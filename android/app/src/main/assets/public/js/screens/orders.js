  ...
  async function renderDetail(app, params) {
    ...
    app.innerHTML = `
      ...
      <div class="page page-pad" style="padding-top:14px;padding-bottom:24px">
        ...
        <div class="card" style="margin-bottom:14px;border-color:var(--status-ready)">
          <button class="btn btn-success btn-block" id="btn-send-whatsapp">${Icons.svg('whatsapp', 18)} <span data-i18n="order.sendWhatsapp"></span></button>
        </div>` : ''}
    ...
    if (whatsappBtn) {
      whatsappBtn.onclick = async () => {
        ...
        const message = I18n.t('whatsapp.readyMessage', { name: order.customer_name, tracking: order.tracking_no, shop: shopName });
