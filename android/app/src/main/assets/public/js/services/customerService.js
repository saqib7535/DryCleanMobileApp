...
const CustomerService = (function () {
  ...
  async function create(data) {
    const res = await DB.run(
      `INSERT INTO customers (name, phone, whatsapp, address, notes, photo_path)
       VALUES (?, ?, ?, ?, ?, ?)`,
  ...
  async function update(id, data) {
    await DB.run(
      `UPDATE customers SET name=?, phone=?, whatsapp=?, address=?, notes=?, photo_path=?, updated_at=datetime('now')
       WHERE id=?`,
      [data.name, data.phone || '', data.whatsapp || '', data.address || '', data.notes || '', data.photo_path || null, id]
    );
  }
