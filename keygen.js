/* ============================================================
   keygen.js — DEV-ONLY product-key generator for DryClean POS.
   ...
   Usage:
     ...
     node keygen.js lifetime 1 "Ali's Laundry"   -> one lifetime key, tagged with a note
...
const SECRET = 'DryCleanPOS-Soothmedia-License-Secret-2026-v1';
...
function generateKey(durationName) {
  ...
  return `DC-${cfg.code}${payload}-${sig}`;
}
...
function ensureLogFile() {
  if (!fs.existsSync(LOG_FILE)) {
    fs.writeFileSync(
      LOG_FILE,
      '# DryClean POS — issued product keys (dev-only record, never ship this file)\n' +
